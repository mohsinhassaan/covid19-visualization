...
* [`src/cljs/covid19_visualization/`](src/cljs/covid19_visualization/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/covid19_visualization/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/covid19_visualization/`](test/cljs/covid19_visualization/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `covid19-visualization` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/covid19_visualization/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns covid19-visualization.example
  (:require [covid19-visualization.config :as config])
